...
class InputHandler {
    constructor() {
        ...
        this.messageContainer = document.querySelector('.comments__user-message');
    ...
    inputSettings(input, sendBtn, limit) {
        if (input && sendBtn && limit) {
            ...
            if (input.value.length > 1000) {
                ...
                let limitAlert = document.querySelector('.user-form__labels_limit-alert');
    ...
    setBasicSettings(input, sendBtn, limit) {
        if (input && sendBtn && limit) {
            input.value = '';
            input.style.height = '61px';
            sendBtn.style.backgroundColor = '#A1A1A1';
